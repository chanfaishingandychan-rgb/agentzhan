...
function getChineseArticleParagraphs(item: AiNewsItem) {
  ...
  return buildCompleteChineseArticle(item, paragraphs);
}

function getArticleSummaryParagraphs(item: AiNewsItem) {
  if (item.category === "模型更新") {
    return [
      `重点是模型能力和使用入口正在变化。${item.takeaway} 简单说，不要只看发布名称，先用自己的真实任务测试，能稳定省时间才值得长期使用。`,
    ];
  }
  if (item.category === "Agent趋势") {
    return [
      `重点是 AI 正在从单纯聊天走向可以连续执行任务的 Agent 形态。${item.takeaway} 真正落地时，先从小流程测试，再考虑接入网站或团队工作。`,
    ];
  }
  if (item.category === "行业应用") {
    return [
      `重点是 AI 已经进入更具体的行业场景，不再只是模型公司之间的参数竞争。${item.takeaway} 对普通人和小团队来说，最重要是把方向变成一个可复制的日常流程。`,
    ];
  }
  if (item.category === "安全与合规") {
    return [
      `重点是 AI 使用越深入，数据、权限和安全边界就越重要。${item.takeaway} 涉及客户资料、健康、财务、代码仓库或企业数据时，必须保留人工复核。`,
    ];
  }
  return [
    `重点是 AI 产品能力继续细分，开始进入更具体的使用场景。${item.takeaway} 建议先确认功能是否开放，再用一个真实任务测试效果。`,
  ];
}

function buildCompleteChineseArticle(item: AiNewsItem, paragraphs: string[]) {
  const context = getArticleContextParagraphs(item);
  const closing = [
    `从这条消息可以看出，${item.source} 这次强调的并不是一个孤立功能，而是 AI 工具继续进入真实工作场景的趋势。对读者来说，最值得看的不是参数本身，而是它会不会改变你选择模型、设计提示词、购买工具或搭建自动化流程的方式。`,
    "因此，这类资讯适合用完整文章读完，而不是只看一句标题。读完后再回到自己的实际场景里判断：它是否已经开放、是否适合我的地区和账号、是否能接入我现有的工具，以及是否真的能节省时间或降低成本。",
  ];

  return [...paragraphs, ...context, ...closing];
}

function getArticleContextParagraphs(item: AiNewsItem) {
  if (item.category === "模型更新") {
    return [
      "模型更新通常会带来两类变化：一类是能力提升，例如更强的推理、更长的上下文、更好的代码生成；另一类是使用方式变化，例如模型名称、API 参数、价格、速度和调用限制。前者影响效果，后者影响能不能稳定用在产品或工作流里。",
      "如果这条消息来自海外模型，用户要特别留意地区开放、账号权限和价格；如果来自大陆模型，就要多看中文能力、本地平台适配、API 稳定性和备案环境下的可用性。不同模型之间不是简单谁更强，而是谁更适合当前任务。",
      "对普通用户来说，最实用的判断方法是拿同一个任务做横向测试。比如同一篇长文，让不同模型总结；同一个客服问题，让不同模型生成回复；同一段代码，让不同模型解释和修改。只有放到真实任务里，模型更新才有实际意义。",
    ];
  }
  if (item.category === "Agent趋势") {
    return [
  ...
  if (item.category === "行业应用") {
    return [
      "行业应用类资讯的重点，不是某家公司说 AI 很强，而是 AI 是否已经进入具体行业流程。真正值得关注的应用，通常会落到一个明确岗位或任务上，例如客服、销售、内容、研发、培训、数据分析或运营。",
      "大陆和国外 AI 资讯在这里各有价值。国外公司往往在底层模型、企业产品和研究方向上释放信号；大陆公司则更接近微信、电商、短视频、本地办公和中文内容生态。两边一起看，才更容易判断未来机会在哪里。",
      "如果你想把这类资讯变成自己的机会，可以先问三个问题：它服务哪个人群，解决哪个重复问题，是否可以低成本试用。只要能回答这三个问题，新闻就不只是资讯，而可能变成一个产品、服务或内容选题。",
    ];
  }
  if (item.category === "安全与合规") {
    return [
      "安全与合规类资讯通常看起来没有模型发布那么热闹，但它决定 AI 能不能进入严肃场景。健康、金融、教育、企业数据和代码仓库都不是随便把资料丢给 AI 就可以处理的地方。",
      "这类消息最应该关注的是数据从哪里来、模型怎样使用资料、用户有没有控制权、输出是否需要人工复核，以及平台有没有明确说明限制。功能越强，责任边界越不能模糊。",
      "对网站和小团队来说，安全合规不一定一开始就做得很复杂，但至少要做到三点：不随便上传敏感资料，不让 AI 自动执行高风险动作，重要结果保留人工检查。",
    ];
  }
  return [
    "产品功能更新通常会改变用户入口。一个功能如果只是名字好听，但不能减少操作步骤、不能改善结果质量、不能接入日常工作，就很难形成长期价值。",
    "判断这类更新时，可以先看它解决的是不是高频问题。高频问题包括写作、搜索、整理资料、生成图片、处理表格、回答客户、制作内容和连接工具。只要能减少其中一个环节的重复劳动，就值得进一步测试。",
    "同时也要留意开放范围。有些功能只在特定地区、特定账号或特定套餐里出现，看到新闻不代表马上可以使用。正式放到网站或工作流前，最好先用自己的账号确认一次。",
  ];
}
